...
function onSortTable(e) {
    if (e.target.classList.contains("sort-order"))
        calculateDissent(0);
    if (e.target.classList.contains("sort-interchange"))
        calculateDissent(1);
    if (e.target.classList.contains("sort-dissent"))
        calculateDissent(2);
    else return;
}
...
function calculateDissent(num) {
    ...
    if ($("callTbody").rows.length > 0) {
        ...
        dissent.sort(function (a, b) {
            switch (num) {
                case 0: return a.order - b.order;
                case 1: return b.interchange - a.interchange;
                case 2: return Math.max(a.givingRight, $("callTbody").rows.length - a.givingRight) -
                    Math.max(b.givingRight, $("callTbody").rows.length - b.givingRight)
                default: return;
            }
        });
...
//$("dissentThead").addEventListener("click", onSortTable)
